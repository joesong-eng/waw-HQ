...
```bash
ssh yd174 "cd /www/wwwroot/iot.tg25.win && git fetch origin main && git reset --hard origin/main && sudo -u www php artisan optimize:clear"
```

### 5.4 Owner with frontend build

```bash
ssh yd174 "cd /www/wwwroot/iot.tg25.win && git fetch origin main && git reset --hard origin/main && pnpm install && pnpm run build && sudo -u www php artisan optimize:clear"
```
