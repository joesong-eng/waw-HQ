...
- `brains/knowledge/03_system_architecture_designs/WAW_2.0_ARCHITECTURE_SPEC.md` - WAW 2.0 核心資料表解耦規格書
- `brains/knowledge/01_agent_governance_rules/DB_MIGRATION_WORKFLOW.md` - 資料庫變更與部署審批規範

### 中關聯（建議讀）
> 了解系統目前的業務流程，建議閱讀

- `brains/knowledge/05_product_and_business_flows/game_v0_arcade/GAME_V0_FLOW_AND_SESSION.md` - 遊戲機 Session 生命週期
- `brains/knowledge/05_product_and_business_flows/kiosk_v0_exchange/KIOSK_EXCHANGE_FLOW.md` - 兌幣機現有通訊流程

### 弱關聯（參考）
> 提供部署參考背景

- `brains/knowledge/04_ops_and_deployments/INFRASTRUCTURE_REFERENCE.md` - 基礎設施主機別名與數據庫配置

### 排除混淆
> 容易混淆但無關的文件

- `brains/knowledge/05_product_and_business_flows/kiosk_v0_exchange/KIOSK_UX_AND_SCREEN_DESIGN.md` - 前端 Kiosk UI 設計，與後端系統架構無關
