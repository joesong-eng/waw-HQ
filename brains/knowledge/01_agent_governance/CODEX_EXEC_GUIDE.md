...
**檔案修改與建立原則**：
- ✅ 優先使用 `exec_command` / bash 指令（例如 `cat << 'EOF' > ...`）直接寫入或修改檔案（注意：`'EOF'` 必須加單引號以防止 Shell 變數提前解析擴展）。
- ✅ 若必須使用 `apply_patch`，請嚴格遵守規格格式以防解析出錯。
...
絕對禁止：
1. 盲目使用易出錯的 `apply_patch` 進行大面積修改（優先使用可靠的 bash 寫入命令）
2. 在 heredoc 寫入時忘記在 EOF 外加單引號（必須使用 `'EOF'`）
3. 自作主張修改超出任務範圍的程式碼
4. 未經 HQ 同意變更系統架構
5. 無限重試同一個失敗操作
6. 修改其他 Agent 的專案檔案
...
- 2026-08-23：調整檔案寫入原則，優先建議使用 `cat << 'EOF'` 以提升修改與寫入的穩定性，並在必須使用 `apply_patch` 時嚴格遵守格式規定。
