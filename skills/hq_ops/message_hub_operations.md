...
hq_task_flow.sh          ← HQ 唯一發令入口
 ├─→ _agent/outbox/to_<Agent>.json    （備份存檔）
...
### 發出後發生的事
1. `_agent/outbox/to_<Agent>.json` 被寫入（備份）
...
### 讀取特定回報
```bash
cat _agent/inbox/<檔名>.json | python3 -m json.tool
```
...
2. `hq_gateway.py` 收到後自動存入 `HQ/_agent/inbox/` 並觸發 DecisionEngine
